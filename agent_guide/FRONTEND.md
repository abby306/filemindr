...
Built on the v2 foundation this cycle: **shell** (hairline top bar + wordmark, blue-wash active nav, neutral account switcher), **upload** (dropzone + segmented pipeline cards + dock). All other screens inherit the tokens but await their structural rework:

- **Archive (REWORK PENDING — the big one):** replace folder-first browse with a **dense sortable table** (title w/ first-page thumbnail via `GET /documents/{id}/pages/1`, folder, status, size, date), instant filtering, folders demoted to filter chips, grid view one toggle away, ⌘K primary nav.
- **Chat (restyle pending):** trace collapses to one quiet mono line; citation chips with amber pin; provenance wash on click-to-source; message bubbles per the rev-2 preview (user = accent fill).
- **Review / Document view / Analytics / Billing (restyle pending):** keep behavior, re-skin; document view keeps SourceGlow (restyled wash, no sweep theatrics).
- **Mobile (pending):** sidebar → bottom tab bar (Upload / Archive / Ask / More), sheets for document view + chat scope, targets ≥44px.
